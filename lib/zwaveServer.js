	  ...
	  var changedNodeid = nodeid
		...
		if (node.ready){
				 ...
				 node.save(function(err){
				    ...
				    if(comclass == 37){
				      ...
				      switch(entityType){
				        case "0x00860x0005":
				          ...
				          if(value.index == 3){
				            eventHandler(node._id, comclass, value["value"], value["units"], "luminance");
				          }
				          if(value.index == 5){
				            eventHandler(node._id, comclass, value["value"], value["units"], "relativeHumidity");
				          }
				          if(value.index == 27){
				            eventHandler(node._id, comclass, value["value"], value["units"], "ultraviolet");
				          }
				          break;
				        default:
				          if(value.index == 1){
				            eventHandler(node._id, comclass, value["value"], value["units"], "temperature");
				          }
				          if(value.index == 3){
				            eventHandler(node._id, comclass, value["value"], value["units"], "luminance");
				          }value["value"]
				    ...
				    if(comclass == 128){
				        eventHandler(node._id, comclass, value["value"], value["units"], "batteryLevel");
				    }

				    if(comclass == 115){
				      if (value.index == 0) {
				        eventHandler(node._id, comclass, value["value"], value["units"], "powerLevel");
				      }
